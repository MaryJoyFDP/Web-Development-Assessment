...
function purchaseButtonClicked() {
    alert('Thank you so much for purchasing!')
...
function addToCartClicked(event) {
    ...
    itemName = itemName.replace(itemPrice, '');
    cartData.push({ "name": itemName, "img": imageSrc, "price": itemPrice });
...
function cartTemplate(prod) {
    ...
    <div class="cart-item"> 
        ...
        <h2 class="cart-name">${prod.name}</h2> 
        <h1 class="cart-price">${prod.price}</h1> 
        <button class="btn button-remove black" type='button'>Remove</button> </div> `;
}
// 
